...
window.addEventListener('load', () => {
    if (window.Adsgram) {
        AdController = window.Adsgram.init({ blockId: "9a1dea9f8d134730875d57f334be6f6e" });
    }
});
...
const MusicMgr = {
    currentTrack: null,
    tracks: ['track1.ogg', 'track2.ogg', 'track3.ogg'], // Твои файлы
    trackIndex: 0,
    enabled: false,
    toggle() {
    ...
    dim(vol) { if (this.currentTrack) this.currentTrack.volume = vol; }
};

const Game = {
    state: { screen: 'splash', level: 1, score: 0, timeLeft: 60, lives: 1, lastTime: 0, invul: 0 },
    ...
    init() {
        ...
        const stop = () => this.ufo.thrust = false;
        ...
        document.getElementById('resume-btn').onclick = () => this.prepareLevel(this.state.screen === 'win' ? this.storage.data.level : this.state.level);
    ...
    storage: {
        ...
        load() {
            const saved = localStorage.getItem('zibbo_v3');
            if(saved) this.data = JSON.parse(saved);
            this.sync();
        },
        save() {
            localStorage.setItem('zibbo_v3', JSON.stringify(this.data));
            this.sync();
        },
        sync() {
        ...
        reset() { localStorage.removeItem('zibbo_v3'); location.reload(); }
    },
    ...
    createStarfield() {
        this.stars = [];
        for(let i=0; i<80; i++) this.stars.push({ x: Math.random()*innerWidth, y: Math.random()*innerHeight, s: Math.random()*2, v: 0.5+Math.random()*2 });
    },

    showScreen(id) {
        document.querySelectorAll('.screen, .hud-layer').forEach(s => s.classList.remove('active'));
        if(id) document.getElementById(id).classList.add('active');
        if(['hud', 'level-screen'].includes(id)) {
    ...
    update(dt) {
        ...
        if(this.ufo.y < 0 || this.ufo.y > this.canvas.height - 40) this.onHit();
        ...
        this.entities.forEach((en, i) => {
            ...
            const dist = Math.hypot((this.ufo.x + 30) - en.x, (this.ufo.y + 20) - en.y);
    ...
    win() {
        ...
        if(this.state.level === this.storage.data.level) this.storage.data.level++;
        this.storage.save();
    ...
    runAdSequence() {
        ...
        const itv = setInterval(() => {
            ...
            if(count <= 0) {
                clearInterval(itv);
                if(AdController) AdController.show().finally(() => this.finishAd());
                else this.finishAd();
            }
        }, 1000);
    },
    ...
    updateLivesUI() {
        ...
        for(let i=1; i<=max; i++) {
            ...
            img.src = `ufo_ship${this.storage.data.currentUfo > 1 ? this.storage.data.currentUfo : ''}.gif`;
    ...
    openShop() {
        ...
        if(this.storage.data.livesPlus < 5) {
            const p = lifePrices[this.storage.data.livesPlus];
            container.innerHTML += `<div class="shop-item"><div class="shop-info"><img src="ufo_ship.gif" class="shop-icon-life"><div><span class="shop-title">+1 LIFE SLOT</span></div></div><button class="shop-buy-btn" onclick="Game.buyLife(${p})">${p} ⭐</button></div>`;
        }
        if(this.storage.data.currentUfo < 5) {
            const next = this.storage.data.currentUfo + 1;
            const p = next * 100;
            container.innerHTML += `<div class="shop-item"><div class="shop-info"><img src="ufo_ship${next}.gif" class="shop-icon-ufo"><div><span class="shop-title">UFO CLASS ${next}</span></div></div><button class="shop-buy-btn" onclick="Game.buyShip(${next}, ${p})">${p} ⭐</button></div>`;
        }
        this.showScreen('shop-screen');
    },

    buyLife(p) { if(this.storage.data.stars >= p) { this.storage.data.stars -= p; this.storage.data.livesPlus++; this.storage.save(); this.openShop(); } },
    buyShip(id, p) { if(this.storage.data.stars >= p) { this.storage.data.stars -= p; this.storage.data.unlockedUfo.push(id); this.storage.data.currentUfo = id; this.storage.save(); this.openShop(); } },
    ...
    draw() {
        ...
        if (this.state.screen !== 'splash') {
            ...
            if (Date.now() < this.state.invul) this.ctx.globalAlpha = Math.sin(Date.now()/50)*0.3+0.6;
            this.ctx.drawImage(ufoImg, -30, -20, 60, 40);
            this.ctx.restore();
        }

        if(this.state.screen === 'waiting') {
            this.ctx.fillStyle = "white"; this.ctx.font = "24px 'Fredoka One'"; this.ctx.textAlign = "center";
